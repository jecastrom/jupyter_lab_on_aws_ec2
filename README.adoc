...
--query "reverse(sort_by(lmages, &CreationDate))[:1].Imageld"
```
=== Provisioning the EC2

```
aws ec2 run-instances --image-id ami-0d527b8c289b4af7f --instance-type t2.micro --subnet-id subnet-0e62e05ae45da43db --count 1 --security-group-id $jp_sg --key-pair create-key-pair
```
